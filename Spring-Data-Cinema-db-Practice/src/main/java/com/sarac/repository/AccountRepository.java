...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account>retreiveAllAdminAccount();

    //Write a JPQL query to sort all accounts with age
    @Query("SELECT a FROM Account a ORDER BY a.age")
    List<Account>retreiveAccountSortedAge();
    ...
    List<Account>retreiveAgeLowerThan( int age);

    //Write a native query to read all accounts that a specific value can be containable in the name, address, country, state city

    @Query(value = "SELECT * FROM account_details WHERE name ILIKE %?1% OR address ILIKE %?1% OR country ILIKE %?1% OR state ILIKE %?1%",nativeQuery = true)
    List<Account> retrieveAllAccountsContainsNameAddressCountryStateCity(String pattern);

    //Write a native query to read all accounts with an age higher than a specific value

    @Query(value = "SELECT *FROM account_details  WHERE age>:age",nativeQuery = true)
    List<Account>retreiveAgeHigherThan(@Param("age") int age);
